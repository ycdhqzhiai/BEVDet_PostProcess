 ...
 * @Last Modified time: 2023-07-10 17:06:18
 */
#pragma once
#include <iostream>
#include <cmath>
...
using out_tensor = std::pair<int, std::vector<float>>;

struct BBox {
  /// Bounding box 3d: {x, y, z, w, h, l, yaw,vel1,vel2}
  // float bbox[9];
  /// Score
  float bbox[7];
...
void xywhr2xyxyr(const float box_xywhr[7], float* dst, float factor) {
  ...
  dst[0] = box_xywhr[0];
  dst[1] = box_xywhr[1];
  dst[2] = box_xywhr[3]*factor;
  dst[3] = box_xywhr[4]*factor;
  dst[4] = box_xywhr[6];
}
